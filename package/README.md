...
**1.0.2 / fonts 1.002.** The tailed `l` is spaced properly: it shipped with no right sidebearing, so it crowded the next letter and above SemiBold its tail crossed into it. The bearing is now tight, with kerning giving space back before `.` `,` `:` `;` `x` `z`. No letterform changed.
